...
from config import PAYMENT_AMOUNT, PAYMENT_DETAILS, ADMIN_CHANNEL_ID, FMD_PAYMENT_AMOUNT, BUNDLE_PAYMENT_AMOUNT, DRY_PAYMENT_AMOUNT
from keyboards.user_kb import (
    ...
    get_dry_payment_keyboard,
    get_dry_days_keyboard,
    get_back_to_dry_days_keyboard,
)
...
from keyboards.callbacks import (
    ...
    DryPaymentCallback, DryDayCallback, DryInfoCallback
)
from data.recipes import (
    get_recipe_text_async, get_available_calories, get_fmd_recipe_text_async,
    get_fmd_shopping_list, get_fmd_info, get_dry_recipe_text_async,
    get_dry_shopping_list, get_dry_info
)
...
class DryPaymentState(StatesGroup):
    """Состояния для процесса оплаты Сушки"""
    waiting_for_screenshot = State()
...
async def cmd_menu(message: Message):
    ...
    has_paid_dry = await db.check_dry_payment_status(user_id)
    ...
    await message.answer(
        ...
        "🔥 <b>Сушка (14 дней)</b>\n"
        "Готовый рацион на ~1500 ккал для снижения жира.",
        reply_markup=get_products_keyboard(
            has_main=has_paid, has_fmd=has_paid_fmd, has_bundle=has_paid_bundle, has_dry=has_paid_dry),
        parse_mode=ParseMode.HTML
    )
...
async def cmd_dry(message: Message):
    """Быстрый доступ к программе Сушка"""
    user_id = message.from_user.id
    has_paid_dry = await db.check_dry_payment_status(user_id)

    if has_paid_dry:
        # Доступ есть - показываем дни Сушки
        await message.answer(
            "🔥 <b>Сушка — Выбери день:</b>\n\n"
            "Программа рассчитана на 14 дней (~1500 ккал).\n"
            "Выбери день, чтобы посмотреть рецепты.",
            reply_markup=get_dry_days_keyboard(),
            parse_mode=ParseMode.HTML
        )
    else:
        # Нет доступа - показываем оплату
        await message.answer(
            f"🔥 <b>Сушка (14 дней)</b>\n\n"
            f"📋 Что это:\n"
            f"• 14-дневная программа питания\n"
            f"• Сбалансированное КБЖУ\n"
            f"• Высокое содержание белка\n"
            f"• Подробные рецепты и список продуктов\n\n"
            f"💰 <b>Стоимость: {DRY_PAYMENT_AMOUNT} ₽</b>",
            parse_mode=ParseMode.HTML
        )
        await show_dry_payment_info(message)
...
async def select_product(callback: CallbackQuery, callback_data: ProductSelectCallback):
    ...
    if product == "main":
        ...
        if has_paid_dry:
            ...
            await callback.message.edit_text(
                f"🔥 <b>Сушка (14 дней)</b>\n\n"
                f"<i>Готовый рацион на ~1500 ккал для снижения процента жира</i>\n\n"
                f"📋 Что это:\n"
                f"• 14-дневная программа питания\n"
                f"• Сбалансированное КБЖУ (высокий белок)\n"
                f"• Подробные рецепты с завтраком, перекусами, обедом и ужином\n"
                f"• Список продуктов\n\n"
                f"💰 <b>Стоимость: {DRY_PAYMENT_AMOUNT} ₽</b>",
                parse_mode=ParseMode.HTML
            )
            await show_dry_payment_info(callback.message)
...
async def show_dry_payment_info(message: Message):
    """Показать информацию для оплаты Сушки"""
    payment_details = PAYMENT_DETAILS.replace('\\n', '\n')

    await message.answer(
        f"💳 <b>Реквизиты для оплаты Сушки:</b>\n\n"
        f"<code>{payment_details}</code>\n\n"
        f"💰 <b>Сумма: {DRY_PAYMENT_AMOUNT} ₽</b>\n\n"
        "⚠️ <b>ВАЖНО:</b>\n"
        "• Внимательно проверьте реквизиты\n"
        "• После оплаты нажмите кнопку ниже\n"
        "• Проверка занимает до 24 часов",
        reply_markup=get_dry_payment_keyboard(),
        parse_mode=ParseMode.HTML
    )


@router.callback_query(DryPaymentCallback.filter())
async def dry_payment_done(callback: CallbackQuery, bot: Bot, state: FSMContext):
    """Пользователь нажал 'Я оплатил(а)' для Сушки - просим скриншот"""
    user = callback.from_user

    # Логируем нажатие кнопки
    await db.log_event(user.id, EventType.PAYMENT_BUTTON_CLICKED, "product:dry")

    # Проверяем, нет ли уже активного запроса на Сушку
    has_pending = await db.has_pending_request(user.id, 'dry')
    if has_pending:
        await callback.answer(
            "⏳ У тебя уже есть запрос на проверке!",
            show_alert=True
    ...
    # Проверяем, не оплачено ли уже
    has_paid_dry = await db.check_dry_payment_status(user.id)
    if has_paid_dry:
        await callback.answer(
            "✅ У тебя уже есть доступ к Сушке!",
            show_alert=True
        )
        return

    # Устанавливаем состояние ожидания скриншота
    await state.set_state(DryPaymentState.waiting_for_screenshot)

    await callback.answer()
    await callback.message.answer(
        "📸 <b>Отправь скриншот оплаты Сушки</b>\n\n"
        "Пожалуйста, отправь фото/скриншот подтверждения перевода.\n"
        "Это поможет модераторам быстрее проверить твою оплату.\n\n"
        "⚠️ <i>На скриншоте должны быть видны: сумма, дата и получатель.</i>",
        reply_markup=get_cancel_payment_keyboard(),
...
@router.message(F.text == "❌ Отмена", DryPaymentState.waiting_for_screenshot)
async def cancel_dry_payment_screenshot(message: Message, state: FSMContext):
    """Отмена отправки скриншота Сушки"""
    await state.clear()
    await message.answer(
        "❌ Отправка отменена.\n\n"
        "Когда будешь готов — нажми кнопку «Я оплатила» снова.",
...
@router.message(F.photo, DryPaymentState.waiting_for_screenshot)
async def receive_dry_payment_screenshot(message: Message, bot: Bot, state: FSMContext):
    """Получение скриншота оплаты Сушки и отправка модераторам"""
    user = message.from_user

    # Логируем отправку скриншота
    await db.log_event(user.id, EventType.SCREENSHOT_SENT, "product:dry")
    ...
    admin_message = await bot.send_photo(
        ...
        photo=photo_file_id,
        caption=(
            "🔔 <b>Новый запрос на проверку оплаты СУШКИ!</b>\n\n"
            f"🔥 <b>Продукт: Сушка ({DRY_PAYMENT_AMOUNT} ₽)</b>\n\n"
            f"👤 Пользователь: {username_display}\n"
            f"📝 Имя: {user.first_name or 'Не указано'}\n"
            f"🆔 ID: <code>{user.id}</code>\n\n"
            "Проверьте оплату и выберите действие:"
    ...
    # Создаём запрос в БД с типом dry
    request_id = await db.create_payment_request(user.id, admin_message.message_id, 'dry')

    # Добавляем кнопки к сообщению админа
    await admin_message.edit_reply_markup(
        reply_markup=get_payment_verification_keyboard(
            user.id, request_id, 'dry')
    )

    await message.answer(
        "✅ <b>Запрос отправлен!</b>\n\n"
        "Скриншот оплаты Сушки передан модераторам.\n"
        "Ты получишь уведомление о результате.\n\n"
        "⏳ Обычно проверка занимает до 24 часов.",
        reply_markup=ReplyKeyboardRemove(),
        parse_mode=ParseMode.HTML
...
@router.message(DryPaymentState.waiting_for_screenshot)
async def wrong_dry_payment_content(message: Message):
    """Неверный формат - ожидаем фото для Сушки"""
    await message.answer(
        "⚠️ <b>Пожалуйста, отправь фото/скриншот оплаты.</b>\n\n"
        "Если хочешь отменить — нажми кнопку «❌ Отмена».",
        parse_mode=ParseMode.HTML
...
@router.callback_query(DryDayCallback.filter())
async def select_dry_day(callback: CallbackQuery, callback_data: DryDayCallback):
    """Выбор дня Сушки и показ рецептов"""
    # Проверяем доступ
    has_paid_dry = await db.check_dry_payment_status(callback.from_user.id)
    if not has_paid_dry:
        await callback.answer("⛔ Сначала оплати доступ к Сушке!", show_alert=True)
        return

    day = callback_data.day
    recipe_text = await get_dry_recipe_text_async(day)

    # Отправляем новое сообщение с рецептами
    await callback.message.answer(
        recipe_text,
        reply_markup=get_back_to_dry_days_keyboard(),
        parse_mode=ParseMode.HTML
    )
    await callback.answer()


@router.callback_query(DryInfoCallback.filter())
async def show_dry_info(callback: CallbackQuery, callback_data: DryInfoCallback):
    """Показ информации о Сушке (список продуктов или описание)"""
    # Проверяем доступ
    has_paid_dry = await db.check_dry_payment_status(callback.from_user.id)
    if not has_paid_dry:
        await callback.answer("⛔ Сначала оплати доступ к Сушке!", show_alert=True)
    ...
    if info_type == "shopping_list":
        text = get_dry_shopping_list()
    elif info_type == "about":
        text = get_dry_info()
    else:
        await callback.answer("Неизвестный тип информации")
        return
...
async def go_back(callback: CallbackQuery, callback_data: BackCallback):
    ...
    elif callback_data.to == "dry_days":
        await callback.message.edit_text(
            "🔥 <b>Сушка — Выбери день:</b>\n\n"
            "Программа рассчитана на 14 дней (~1500 ккал).\n"
            "Выбери день, чтобы посмотреть рецепты.",
