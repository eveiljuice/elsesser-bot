...
async def cmd_start(message: Message):
    ...
    if has_paid:
        ...
        # Промо-сообщение FMD-Detox для новых пользователей
        await message.answer(
            "🎄 <b>FMD-Detox — мой первый подарок себе уже после боя курантов!</b>\n\n"
            "Встречу 1-е января в любимом платье, а проведу всю следующую неделю в новом теле — "
            "без отёков, тяжести и «праздничного» целлюлита.\n\n"
            "Программа стартует 3 января в 9:00 — ровно тогда, когда холодильник перестанет манить, "
            "а мотивация ещё пульсирует.\n\n"
            "🥗 <b>Что внутри моего послерождественского детокса:</b>\n\n"
            "• Без голодовок — 3 разноцветных приёма пищи в день, калорийность плавает, "
            "но никогда не опускается ниже базового обмена.\n\n"
            "• Секретные схемы слива воды — вечерние «минеральные окна» и утренние «лимонные старты», "
            "которым я сама удивлюсь.\n\n"
            "• Рецепты из тех продуктов, что уже в холодильнике — капуста, свёкла, яйца, кефир, апельсины. "
            "Ни одной «суперфуд-одеялки».\n\n"
            "• Вегетарианская ветка — есть и для меня, и для подруги, которая на посту.\n\n"
            "✨ <b>Итог через 5 дней:</b> –3 см в талии, –2 см в бёдрах, кожа без «апельсина», "
            "лицо как после 8-часового сна.\n\n"
            "Фото с ёлкой уже в архиве — новые будут с построенным прессом.\n\n"
            "Кто со мной? Жми кнопку ниже и встречай весёлое «после» вместе со мной 👇",
            reply_markup=get_fmd_promo_keyboard(),
            parse_mode=ParseMode.HTML
        )
...
async def cmd_help(message: Message):
    ...
    await message.answer(
        ...
        "/menu — Выбрать рацион питания\n"
        "/fmd — Оплатить FMD Протокол\n"
...
async def handle_command_button(callback: CallbackQuery):
    ...
    if command == "/start":
        ...
        if has_paid:
            ...
            await callback.message.answer(
                f"👋 <b>Привет, {user.first_name}!</b>\n\n"
                "🍽 Я помогу тебе с <b>готовыми рационами питания</b> "
                "на каждый день с точным расчётом калорий и КБЖУ!\n\n"
                "✨ <b>Что внутри:</b>\n"
                "• Рационы от 1200 до 2100 ккал\n"
                "• Завтрак, обед и ужин на 14 дней\n"
                "• Подробные рецепты с составом\n"
                "• FMD протокол (диета имитирующая голодание)\n\n"
                "💡 Не нужно считать калории — всё уже рассчитано!\n\n"
